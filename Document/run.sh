...
do
	...
	if [ ! -f tmp/texts/$ofile ]; then
		...
		python3 preprocess_estadao.py $ofile
	    elif [[ $ofile == *"folha"* ]]; then
		python2 boilerpipe_gettext.py $ofile
		python3 preprocess_folha.py $ofile
            elif [[ $ofile == *"clarin"* ]]; then
		python2 boilerpipe_gettext.py $ofile
		python3 preprocess_clarin.py $ofile
            elif [[ $ofile == *"lanacion"* ]]; then
		python2 boilerpipe_gettext.py $ofile
		python3 preprocess_lanacion.py $ofile
            elif [[ $ofile == *"pagina12"* ]]; then
		python2 boilerpipe_gettext.py $ofile
		python3 preprocess_pagina12.py $ofile
